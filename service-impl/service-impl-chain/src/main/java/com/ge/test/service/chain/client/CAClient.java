 ...
 */       
package com.ge.test.service.chain.client;
...
//	}
	 ...
	 * @throws Exception
	 */
	public UserContext enrollAdminUserTLS(String fabricUserPath, String username, String password) throws Exception {
		
		UserContext userContext = Util.readUserContext(fabricUserPath, adminContext.getAffiliation(), username);
		if (userContext != null) {
			Logger.getLogger(CAClient.class.getName()).log(Level.WARNING, "CA -" + caUrl + " admin is already enrolled.");
			return userContext;
		}
        ...
        EnrollmentRequest enrollmentRequestTLS  = new EnrollmentRequest();
 		enrollmentRequestTLS.addHost(FabricConfig.CA_ORG1_URL);
 		enrollmentRequestTLS.setProfile("tls");
 		Enrollment adminenroll = instance.enroll(username, password, enrollmentRequestTLS);
		// 注册之后有 私钥 和 证书
//		adminenroll.getKey().getEncoded();
//		adminenroll.getCert();
		adminContext.setEnrollment(adminenroll);
		Logger.getLogger(CAClient.class.getName()).log(Level.INFO, "CA -" + caUrl + " Enrolled Admin.");
		Util.writeUserContext(fabricUserPath, adminContext);
		return adminContext;
	}

//	public UserContext getEnrollAdminUserTLS(String username, String password) throws Exception {
//		UserContext userContext = Util.readUserContext(adminContext.getAffiliation(), username);
//		if (userContext != null) {
//			Logger.getLogger(CAClient.class.getName()).log(Level.WARNING, "CA -" + caUrl + " admin is already enrolled.");
//			return userContext;
